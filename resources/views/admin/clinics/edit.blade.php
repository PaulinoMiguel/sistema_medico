<x-layouts.admin :title="'Editar Clínica'">
    <div class="mb-6">
        <a href="{{ route('admin.clinics.index') }}" class="text-blue-400 hover:underline text-sm">&larr; Volver a clínicas</a>
        <h2 class="text-2xl font-bold text-white mt-2">Editar Clínica</h2>
    </div>
    ...
    <form method="POST" action="{{ route('admin.clinics.update', $clinic) }}">
        ...
        <div class="bg-gray-800 rounded-lg shadow p-6 mb-6">
            <h3 class="text-lg font-semibold text-white mb-4">Datos de la clínica</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="is_active" class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:ring-blue-500 focus:border-blue-500">
                        <option value="1" {{ old('is_active', $clinic->is_active) ? 'selected' : '' }}>Activa</option>
                        <option value="0" {{ !old('is_active', $clinic->is_active) ? 'selected' : '' }}>Inactiva</option>
                    </select>
                </div>
                <div>
