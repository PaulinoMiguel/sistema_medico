    ...
    <form method="POST" action="{{ route('patients.update', $patient) }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="mb-6 flex items-center gap-4">
                <div id="photo-preview" class="w-20 h-20 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                    @if($patient->photo_url)
                        <img src="{{ $patient->photo_url }}" class="w-20 h-20 rounded-full object-cover">
                    @else
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">JPG, PNG o WEBP. Máximo 2MB. Dejar vacío para mantener la foto actual.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="gestational_age_weeks" min="22" max="44"
                           ...
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                           placeholder="40">
                    <p class="mt-1 text-xs text-gray-500">Solo pediatría. &lt;37 usa edad corregida en curvas.</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="document_type" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="cedula" {{ old('document_type', $patient->document_type) == 'cedula' ? 'selected' : '' }}>Cédula</option>
                ...
                <div>
                    ...
                    <select name="insurer_id" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($insurers as $ins)
                            <option value="{{ $ins->id }}" {{ old('insurer_id', $patient->insurer_id) == $ins->id ? 'selected' : '' }}>{{ $ins->name }}</option>
                        @endforeach
                    </select>
                    @if(!$patient->insurer_id && $patient->insurance_provider)
                        <p class="mt-1 text-xs text-amber-600">Antes: "{{ $patient->insurance_provider }}" (texto). Elige la aseguradora del catálogo.</p>
                    @endif
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">No. póliza</label>
                    <input type="text" name="insurance_policy_number" value="{{ old('insurance_policy_number', $patient->insurance_policy_number) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="tel" name="secondary_phone" value="{{ old('secondary_phone', $patient->secondary_phone) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('notes', $patient->notes) }}</textarea>
                </div>
            </div>
        </div>
